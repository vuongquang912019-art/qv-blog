...
export default async function Home(){
  ...
  return(
    <main className={`min-h-screen theme-${settings.theme}`} style={{background:'var(--bg)', color:'var(--text)'}}>
      ...
      <section className="max-w-[1240px] mx-auto px-6 pt-4 pb-12"><div className="inline-flex gap-2 px-4 py-2 rounded-full bg-white text-xs mb-8">{settings.badge_text} • {posts.length} bai • Theme: {settings.theme}</div><h1 className="font-bold text-[56px] leading-[0.85]">{settings.hero_line1}<br/><span className="opacity-60">{settings.hero_line2}</span></h1><p className="mt-6 opacity-70 max-w-xl">{settings.hero_desc}</p></section>
